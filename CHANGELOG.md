...
- **Colours from Helix themes.** Theme files are read from `~/.config/leotui/themes`, then `~/.config/helix/themes`, then `$HELIX_RUNTIME/themes`, and none is vendored. Helix keys a theme by tree-sitter capture name, so a theme applies to the highlighter's classes as it is. The default is `sonokai`; without it, the terminal's 16 colours are used.
...
- **`~/.config/leotui/config.toml`**, holding `theme`. Accepting `:theme NAME` saves it there, rewriting only that line; `--theme NAME` sets a theme for one launch without saving. The file is TOML, read by a hand parser: the format has three shapes, against five crates for `toml` and `serde`.
